(() => {
  ...
  async function testGcpAuth(){try{const r=await api('/api/science?op=gcp_auth_probe');out('Google WIF',r)}catch(e){out('Error Google WIF',e.message)}}
  ...
  window.addEventListener('DOMContentLoaded',()=>{$('#prepare-physics-job').addEventListener('click',prepareJob);$('#test-gcp-auth').addEventListener('click',testGcpAuth);$('#preview-gcp-job').addEventListener('click',previewGcp);$('#submit-gcp-job').addEventListener('click',submitGcp);$('#list-gcp-outputs').addEventListener('click',listGcpOutputs);$('#get-gcp-manifest').addEventListener('click',getGcpManifest);$('#run-physics-router').addEventListener('click',runRouter);$('#run-monte-carlo').addEventListener('click',runMonteCarlo);$('#run-nvidia-chat').addEventListener('click',runChat);$('#run-capability').addEventListener('click',runCapability);$('#clear-tools-output').addEventListener('click',()=>$('#tools-output').textContent='ScientificBrain Physics Tools listo.');load().catch(e=>out('Inicialización',e.message))});
})();